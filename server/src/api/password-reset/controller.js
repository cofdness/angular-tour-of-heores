...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your heroes api account.<br>
        ...
        &mdash; heroes api Team
      `
      return sendMail({ toEmail: email, subject: 'heroes api - Password Reset', content })
    })
